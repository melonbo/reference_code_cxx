...
void VideoPlayer::setSpeed(float coef)
{
    speed_coefficient = coef;
    closeSDL();
    openSDL();
}
...
int VideoPlayer::openSDL()
{
    ...
    wanted_spec.freq = samplerate*speed_coefficient;
    ...
    qDebug("find %d audio device", num);
    for (int i=0;i<num;i++)
    {
        qDebug()<<"SDL_GetAudioDeviceName:"<<SDL_GetAudioDeviceName(i,0);
...
//    char file_path_x[] = "rtsp://admin:waycom12345@172.16.41.1:554/h264/ch1/main/av_stream";
    char file_path_x[] = "http://localhost:80/earth01.mp4";
    if (avformat_open_input(&pFormatCtx, file_path_x, nullptr, nullptr) != 0)
    {
