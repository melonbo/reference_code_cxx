    ...
    ui(new Ui::VideoPlayerWidget)
{
    ...
    connect(ui->pushButton_speed, &QPushButton::clicked, this, &VideoPlayerWidget::slotBtnClick);
...
void VideoPlayerWidget::slotBtnClick(bool isChecked)
{
    ...
    else if(QObject::sender() == ui->pushButton_speed)
    {
        ui->pushButton_speed->setStyleSheet("QPushButton#pushButton_speed{border-image: url(:image/speed_x2.png)}");
        mPlayer->pause();
        mPlayer->setSpeed(1.2);
        mPlayer->play();
    }
...
void VideoPlayerWidget::keyPressEvent(QKeyEvent *event)
{
    switch(event->key())
    {
     ...
     default:
        VideoPlayerWidget::keyPressEvent(event);
    }
}
