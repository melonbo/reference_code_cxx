...
char* RTSPServer::rtspURLPrefix(int clientSocket) const {
  char addr[20];
  get_local_ip(platform->eth_name, addr);
...
//  }
  ourAddress.sin_addr.s_addr = inet_addr(addr);
...
printf("function incomingConnectionHandlerHTTP(argv)\n");
  RTSPServer* server = (RTSPServer*)instance;
  server->incomingConnectionHandlerHTTP();
}
void RTSPServer::incomingConnectionHandlerHTTP() {
printf("function incomingConnectionHandlerHTTP()\n");
...
printf("function noteTCPStreamingOnSocket()\n");
...
printf("function unnoteTCPStreamingOnSocket(), socketNum=%d\n",socketNum);
...
printf("function stopTCPStreamingOnSocket()\n");
...
printf("function RTSPClientConnection(), constructed\n");
  resetRequestBuffer();
}
...
printf("function handleCmd_SET_PARAMETER()\n");
...
printf("function handleCmd_DESCRIBE()\n");
...
printf("function handleCmd_notSupported()\n");
  snprintf((char*)fResponseBuffer, sizeof fResponseBuffer,
...
printf("function parseHTTPRequestString()\n");
...
printf("function handleHTTPCmd_notSupported()\n");
  snprintf((char*)fResponseBuffer, sizeof fResponseBuffer,
...
printf("function handleHTTPCmd_notFound()\n");
  snprintf((char*)fResponseBuffer, sizeof fResponseBuffer,
...
void RTSPServer::RTSPClientConnection::handleHTTPCmd_OPTIONS() {
    printf("function handleHTTPCmd_OPTIONS()\n");
#ifdef DEBUG
...
void RTSPServer::RTSPClientConnection::handleHTTPCmd_TunnelingGET(char const* sessionCookie) {
    printf("function handleHTTPCmd_TunnelingGET()\n");
...
::handleHTTPCmd_TunnelingPOST(char const* sessionCookie, unsigned char const* extraData, unsigned extraDataSize) {
    printf("function handleHTTPCmd_TunnelingPOST()\n");
...
void RTSPServer::RTSPClientConnection::closeSocketsRTSP() {
    printf("function closeSocketsRTSP()\n");
...
void RTSPServer::RTSPClientConnection::handleAlternativeRequestByte(void* instance, u_int8_t requestByte) {
    printf("function handleAlternativeRequestByte()\n");
...
printf("function handleAlternativeRequestByte1()")    ;
  if (requestByte == 0xFF) {
...
void RTSPServer::RTSPClientConnection::handleRequestBytes(int newBytesRead) {
    printf("function handleRequestBytes()\n");
...
static Boolean parseAuthorizationHeader(char const* buf,
					...
					char const*& response) {
    printf("function parseAuthorizationHeader()\n");
...
::authenticationOK(char const* cmdName, char const* urlSuffix, char const* fullRequestStr) {
    printf("function authenticationOK()\n");
...
::setRTSPResponse(char const* responseStr) {
    printf("function setRTSPResponse()\n");
  snprintf((char*)fResponseBuffer, sizeof fResponseBuffer,
	   "RTSP/1.0 %s\r\n"
	   "CSeq: %s\r\n"
	   "%s\r\n",
...
::changeClientInputSocket(int newSocketNum, unsigned char const* extraData, unsigned extraDataSize) {
    printf("function changeClientInputSocket()\n");
...
::RTSPClientSession(RTSPServer& ourServer, u_int32_t sessionId)
  : GenericMediaServer::ClientSession(ourServer, sessionId),
    ...
    printf("function RTSPClientSession()\n");
}
...
void RTSPServer::RTSPClientSession::deleteStreamByTrack(unsigned trackNum) {
    printf("fuction RTSPServer::RTSPClientSession::deleteStreamByTrack()\n");
...
void RTSPServer::RTSPClientSession::reclaimStreamStates() {
    printf("function RTSPServer::RTSPClientSession::reclaimStreamStates()\n");
  for (unsigned i = 0; i < fNumStreamStates; ++i) {
    if (fStreamStates[i].subsession != NULL) {
...
static void parseTransportHeader(char const* buf,
    ...
    printf("function parseTransportHeader()\n");
  // Initialize the result parameters to default values:
...
static Boolean parsePlayNowHeader(char const* buf) {
    printf("function parsePlayNowHeader()\n");
...
::handleCmd_SETUP(RTSPServer::RTSPClientConnection* ourClientConnection,
    ...
    printf("function handleCmd_SETUP()\n");
...
::handleCmd_withinSession(RTSPServer::RTSPClientConnection* ourClientConnection,
    ...
    printf("function handleCmd_withinSession()\n");
  // This will either be:
...
::handleCmd_TEARDOWN(RTSPServer::RTSPClientConnection* ourClientConnection,
		     ServerMediaSubsession* subsession) {
    printf("function handleCmd_TEARDOWN()\n");
  unsigned i;
...
::handleCmd_PLAY(RTSPServer::RTSPClientConnection* ourClientConnection,
    ...
    printf("function handleCmd_PLAY()\n");
  char* rtspURL
...
::handleCmd_PAUSE(RTSPServer::RTSPClientConnection* ourClientConnection,
		  ServerMediaSubsession* subsession) {
    printf("function handleCmd_PAUSE()\n");
  for (unsigned i = 0; i < fNumStreamStates; ++i) {
    if (subsession == NULL /* means: aggregated operation */
	|| subsession == fStreamStates[i].subsession) {
      if (fStreamStates[i].subsession != NULL) {
