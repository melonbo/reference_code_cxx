...
soap_wsa_reply(struct soap *soap, const char *id, const char *action)
{
  ...
  /* WCF Interoperability:
     indexInstance is required when the WCF Application hosts multiple
     Callback indexs within the same application. The
     ReferenceParameters->indexInstance element serves as a hint to the WCF
     Client dispatcher, as to which WCF callback instance a received SOAP
     Envelope belongs to indexInstance is declared as a pointer, so it is
     essentially an optional element. Tests with Applications not requiring
     indexInstance have also been done for the following fix.
  */
  if (oldheader && oldheader->SOAP_WSA(ReplyTo) && oldheader->SOAP_WSA(ReplyTo)->ReferenceParameters && oldheader->SOAP_WSA(ReplyTo)->ReferenceParameters->chan__indexInstance)
  {
    if (!newheader->chan__indexInstance)
    {
      newheader->chan__indexInstance = (struct chan__indexInstanceType*)soap_malloc(soap, sizeof(struct chan__indexInstanceType));
      if (newheader->chan__indexInstance)
      {
	soap_default_chan__indexInstanceType(soap, newheader->chan__indexInstance);
	newheader->chan__indexInstance->__item = *(oldheader->SOAP_WSA(ReplyTo)->ReferenceParameters->chan__indexInstance);
	newheader->chan__indexInstance->wsa5__IsReferenceParameter = _wsa5__IsReferenceParameter__true;
      }
    }
    else
    {
