...
//构造函数
CameraConfig::CameraConfig(char index)
{
...
//                    printf("[%s]XAddr_ip is %s\n", camera_IP, XAddr_ip);
                ...
                if(strcmp(camera_IP, XAddr_ip) == 0)
                {
                    ...
                    printf("[%s]current device %s is online\n", camera_IP, camera_IP);
                    break;
                }
            }
        }
    ...
    *(tds__GetCapabilities->Category) = (enum tt__CapabilityCategory)(tt__CapabilityCategory__Media);
    ...
    if(!soap->error)
    {
        if(tds__GetCapabilitiesResponse->Capabilities->Media->XAddr != NULL)
        {
            ...
            printf("[%s]get media addr success, mediaAddr is %s\n", camera_IP, camera_mediaAddr);
            printf("[%s]get media addr success, eventAddr is %s\n", camera_IP, camera_eventAddr);
        }
        else
        {
            mprintf("[%s]getCapabilities error, soap->error = %d\n", camera_IP, soap->error);
        }
    }
    else
    {
...
//            printf("[%s]trt__GetProfilesResponse->Profiles[%d].VideoEncoderConfiguration->token = %s\n", camera_IP, i, trt__GetProfilesResponse->Profiles[i].VideoEncoderConfiguration->token);
            ...
            mprintf("[%s]camera %d config token is %s\n", camera_IP, i, camera_token[i].configToken);
            mprintf("[%s]camera %d profile token is %s\n", camera_IP, i, camera_token[i].profileToken);
            mprintf("[%s]camera %d videosource token is %s\n", camera_IP, i, camera_token[i].videoSourceToken);
        }
    }
    else
    {
        mprintf("[%s]SOAP error string: %s\n", camera_IP, *soap_faultstring(soap));
        mprintf("[%s]SOAP error detail: %s\n", camera_IP, *soap_faultdetail(soap));
        mprintf("[%s]getProfiles() failed, soap->error = %d\n", camera_IP, soap->error);
        //get profile failed
...
int CameraConfig::getStreamUri()
{
    ...
    for(i = 0; i < camera_info.camera_configurationSize; i++)
    {
        ...
        {
            mprintf("[%s]getStreamUri error, soap->error = %d\n", camera_IP, soap->error);
...
//                printf("[%s]getOptionsResponse.Options->Extension->H264->BitrateRange = (%d ~ %d)\n", camera_IP, trt__GetVideoEncoderConfigurationOptionsResponse->Options->Extension->H264->BitrateRange->Min, trt__GetVideoEncoderConfigurationOptionsResponse->Options->Extension->H264->BitrateRange->Max);
        ...
        {
            mprintf("[%s]get configuration option error, soap->error = %d\n", camera_IP, soap->error);
...
//        printf("[%s]trt__GetOSDOptionsResponse->OSDOptions->MaximumNumberOfOSDs->Image = %d\n", camera_IP, trt__GetOSDOptionsResponse->OSDOptions->MaximumNumberOfOSDs->Image);
    ...
    {
        mprintf("[%s]GetOSDOptions error %d\n", camera_IP, soap->error);
...
int CameraConfig::make_uri_withauth(char *src_uri, char *username, char *password, char *dest_uri, unsigned int size_dest_uri)
{
    ...
    if (size_dest_uri < needBufSize) {
        mprintf("[%s]dest uri buf size is not enough.\n", camera_IP);
        result = -1;
        goto EXIT;
    }
    ...
    if (0 == strlen(username) && 0 == strlen(password)) {                       // �����µ�uri��ַ
        ...
        if (NULL == p) {
            mprintf("[%s]can't found '//', src uri is: %s.\n", camera_IP, src_uri);
            result = -1;
            goto EXIT;
        }
        p += 2;
...
int CameraConfig::setVideoConfig(int num)
{
    ...
    {
        mprintf("[%s]setVideoConfig error, soap->error = %d\n", camera_IP, soap->error);
...
//                soap_error_number = soap->error;
            ...
            else //we find a device
            {
                ...
                if(result == NULL)
                {
                    mprintf("strtok result is null, tmpAddr: %s\n", tmpAddr);
                    continue;
                }
                printf("result is %s, strlen of result is %d\n", result, strlen(result));
                memset(&(tmp[count].ip), '\0', strlen(result)+1);
...
int CameraConfig::getSystemDateAndTime()
{
    ...
    if(!soap->error)
    {
        ...
        printf("[%s]camera time is %04d-%02d-%02d %02d:%02d:%02d\n", camera_IP, year, month, day, hour, min, sec);
    }
    else
    {
        mprintf("[%s]getSystemDateAndTime error, soap->error = %d\n", camera_IP, soap->error);
...
int CameraConfig::setSystemDateAndTime(int sysYear, int sysMonth, int sysDay, int sysHour, int sysMinute, int sysSecond)
{
    ...
    if(!soap->error)
    {
        ...
        int min = rep.SystemDateAndTime->LocalDateTime->Time->Minute;
        int sec = rep.SystemDateAndTime->LocalDateTime->Time->Second;
        printf("[%s]time of camera is %04d-%02d-%02d %02d:%02d:%02d\n", camera_IP, year, month, day, hour, min, sec);
    }
    else
    {
...
//    req_set.TimeZone->TZ = rep.SystemDateAndTime->TimeZone->TZ;
    ...
    {
        mprintf("[%s]set camera time failed, error = %d\n", camera_IP, soap->error);
        mprintf("[%s]SOAP error string: %s\n", camera_IP, *soap_faultstring(soap));
        mprintf("[%s]SOAP error detail: %s\n", camera_IP, *soap_faultdetail(soap));
...
**返回：
        0表明未找到，非0表明找到
************************************************************************/
int CameraConfig::find_event(struct _tev__PullMessagesResponse *rep, char *topic, char *name, char *value)
{
    mprintf("[%s]get into find_event() ...\n", camera_IP);
    int i, j;
...
//        }
        ...
        if(strstr(p->Message.__any, "IsMotion") > 0)
        {
            if(strstr(p->Message.__any, "true") > 0)
            {
//                printf("[%s]message %d : %s\n", camera_IP, i, p->Message.__any);
//                printf("[%s]motion detect active ...\n", camera_IP);
            ...
            if(strstr(p->Message.__any, "false") > 0)
            {
//                printf("[%s]message %d : %s\n", camera_IP, i, p->Message.__any);
//                printf("[%s]motion detect stop\n", camera_IP);
        ...
        else if(strstr(p->Topic->__any, "MotionAlarm") > 0)
        {
//            printf("[%s]message %d : %s\n", camera_IP, i, p->Topic->__any);
            //motion continue ...
            return 2;
...
int CameraConfig::ONVIF_CreatePullPointSubscription(const char *EventXAddr)
{
    ...
    if(strcmp(camera_factory, "hk") == 0)
        PULLMSG_TIMEOUT_UNIT = 1000;// 海康IPC
    else if(strcmp(camera_factory, "dahua") == 0)
...
int CameraConfig::setCameraAlarmOsd(std::string alarm_text)
{
    int ret;
    ...
    mprintf("[%s]create osd text : %s\n", camera_IP, alarm_text.data());
    setOSDString(alarm_text.data());
    ret = createOSD();
    if(ret == 0)
    {
