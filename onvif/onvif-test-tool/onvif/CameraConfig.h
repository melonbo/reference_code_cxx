...
public:
    ...
    */
    ...
    int  setCameraAlarmOsd(std::string alarm_text);

    int isAlarm;
    int isAlarmVideoList;
    unsigned char alarm_type;//bit0:emergency alarm, bit1:pre fire, bit2:fire, bit3:door unlock bit4:door close block bit5:door open block
    unsigned char alarm_type_old;

    /*错误代码*/
    int soap_error_number;
    char alarm_type_name[8][20]={
        "EMERGEN",
        "PRE_FIRE",
        "FIRE",
        "DOOR_UNLOCK",
